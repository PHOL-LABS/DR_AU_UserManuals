\chapter{Situaciones típicas al configurar \DRAudi{}}\label{ch:replica-scenarios}

Antes de realizar un diagnóstico, confirme que el cuadro corresponde al \DRAudi{} clásico (\autoref{ch:replica-setup}). Los paneles \DRAudi{} utilizan un portal Wi-Fi y se describen en \autoref{ch:replica-next-scenarios}.

\begin{description}
    \item[Módulo Bluetooth no detectado] Empareje con la interfaz Bluetooth Classic del cuadro (normalmente se anuncia como \texttt{DRAudi}). Serial Bluetooth Terminal para Android sigue siendo la herramienta recomendada: configure el carácter de fin de línea como LF y evite los escáneres solo BLE, que no pueden descubrir el módulo.
    \item[El iPhone o iPad no se conecta] Los cuadros \DRAudi{} utilizan Bluetooth~2.0 y no son compatibles con dispositivos iOS. Use un teléfono Android o un ordenador con una utilidad de puerto serie Bluetooth.
